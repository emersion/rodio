...
// TODO
unsafe impl<R> Send for Mp3Decoder<R> where R: Read + Seek {}
    ...
    where R: Read + Seek
{
    ...
    pub fn new(mut data: R) -> Result<Mp3Decoder<R>, R> {
        ...
        let mut reader = Decoder::decode(data).unwrap();
...
impl<R> Iterator for Mp3Decoder<R> where R: Read + Seek {
    type Item = i16;
    ...
    fn next(&mut self) -> Option<i16> {
        ...
        let sample = self.current_frame.samples[self.current_frame_channel][self.current_frame_sample_pos].to_i16();
...
fn next_frame<R>(decoder: &mut Decoder<R>) -> Frame where R: Read {
    decoder.get_frame().unwrap_or_else(|_| {
        Frame {
            sample_rate: 44100,
            samples: vec![Vec::new()],
            ...
            bit_rate: 0,
            layer: simplemad_sys::MadLayer::default(),
            mode: simplemad_sys::MadMode::default(),
        }
    })
}
    ...
    where R: Read + Seek
{
    ...
    if Decoder::decode(data.by_ref()).is_err() {
